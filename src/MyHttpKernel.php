 ...
 */
class MyHttpKernel implements HttpKernelInterface, TerminableInterface
{
    ...
    public function __construct($app)//EventDispatcherInterface $dispatcher, ControllerResolverInterface $resolver, RequestStack $requestStack = null, ArgumentResolverInterface $argumentResolver = null)
    {

        $this->app = $app;
        $this->dispatcher = $app['dispatcher'];
        $this->resolver = $app['resolver'];
        $this->requestStack = $app['request_stack'] ?: new RequestStack();
        ...
        if (null === $this->argumentResolver) {
            ...
            $this->argumentResolver = $app['resolver'];
        }
    }
    ...
    private function handleRaw(Request $request, $type = self::MASTER_REQUEST)
    {
        ...
        if (is_array($controller)) {
            if (method_exists($controller[0], 'init')) $controller[0]->init($this->app);
            if (method_exists($controller[0], 'checkAccess')) {
                if ($controller[0]->checkAccess($this->app['user'], $controller[1])) {
                    $response = \call_user_func_array($controller, $arguments);
                } else {
                    //Return some Response with err
                    $response = new Response('No access');
                }
            } else {
                $response = \call_user_func_array($controller, $arguments);
            }
